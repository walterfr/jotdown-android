...
class ReaderViewModel(
    ...
    // Expoe o File diretamente para o ReaderScreen
    ...
    private val _strokeColor = MutableStateFlow(0xFFFDE047.toInt())
    ...
    init {
        Log.d("Jotdown", "ReaderViewModel init — documentId: $documentId")
        viewModelScope.launch {
            val doc = repository.getDocumentById(documentId)
            Log.d("Jotdown", "Documento: ${doc?.title} — path: ${doc?.pdfFilePath}")
            _document.value = doc
        }
    }
    ...
    fun addAnnotation(page: Int, x: Float, y: Float, text: String) {
        viewModelScope.launch {
            repository.saveAnnotation(
                AnnotationEntity(id = System.currentTimeMillis(), documentId = documentId,
    ...
    fun updateAnnotation(id: Long, text: String) {
        viewModelScope.launch {
            val existing = annotations.value.find { it.id == id } ?: return@launch
            repository.saveAnnotation(existing.copy(text = text))
        }
    }

    fun deleteAnnotation(id: Long) {
        viewModelScope.launch { repository.deleteAnnotation(id) }
    }

    fun addHighlight(page: Int, text: String) {
        viewModelScope.launch {
            repository.saveHighlight(
                HighlightEntity(id = System.currentTimeMillis(), documentId = documentId,
                    page = page, text = text)
            )
        }
    }
    ...
    fun saveDrawing(page: Int, pathsJson: String) {
        viewModelScope.launch {
            repository.saveDrawing(
                DrawingEntity(documentId = documentId, page = page, pathsJson = pathsJson)
            )
        }
    }

    fun saveMetadata(
        docType: String, authorLastName: String, authorFirstName: String,
        title: String, subtitle: String, edition: String, city: String,
        publisher: String, year: String, journal: String, volume: String,
        pages: String, url: String, accessDate: String
    ) {
        viewModelScope.launch {
